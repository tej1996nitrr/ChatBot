...
def train(train_loader, transformer, criterion, epoch):
    ...
    transformer.train() #dropout acts differently in training and testing so we initialize as train
    sum_loss = 0
    count = 0
    ...
    for i, (question, reply) in enumerate(train_loader):
        ...
        # Prepare Target Data
        reply_input = reply[:, :-1] #last token is end token so se dont include that
        reply_target = reply[:, 1:] #skip start token 
